...
public class TransactionController {
    ...
    public ResponseEntity<CollectionModel<TransactionDto>> getTransactionsByReceiverId(@PathVariable Integer receiverId) {
        ...
        Link transactionsLink = linkTo(methodOn(TransactionController.class).getTransactionsByReceiverId(receiverId)).withRel("transactions");
        CollectionModel<TransactionDto> transactionDtos = transactionDtoAssembler.toCollectionModel(transactions, transactionsLink);
        return new ResponseEntity<>(transactionDtos, HttpStatus.OK);
    }
    ...
    public ResponseEntity<CollectionModel<TransactionDto>> getTransactionsBySenderId(@PathVariable Integer senderId) {
        ...
        Link transactionsLink = linkTo(methodOn(TransactionController.class).getTransactionsBySenderId(senderId)).withRel("transactions");
        CollectionModel<TransactionDto> transactionDtos = transactionDtoAssembler.toCollectionModel(transactions, transactionsLink);
        return new ResponseEntity<>(transactionDtos, HttpStatus.OK);
    }
