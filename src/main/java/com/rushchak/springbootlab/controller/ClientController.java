...
import com.rushchak.springbootlab.dto.assembler.ClientDtoAssembler;
...
public class ClientController {
    ...
    @GetMapping(value = "/ofHomeAddress/{homeAddressId}")
    public ResponseEntity<CollectionModel<ClientDto>> getClientsByHomeAddressId(@PathVariable Integer homeAddressId) {
        List<Client> clients = clientService.findClientsByHomeAddressId(homeAddressId);
        Link clientsLink = linkTo(methodOn(ClientController.class).getClientsByHomeAddressId(homeAddressId)).withRel("clients");
        CollectionModel<ClientDto> clientDtos = clientDtoAssembler.toCollectionModel(clients, clientsLink);
        return new ResponseEntity<>(clientDtos, HttpStatus.OK);
    }
    ...
    public ResponseEntity<?> updateClient(@RequestBody Client updClient, @PathVariable Integer clientId) {
        clientService.update(clientId, updClient);
